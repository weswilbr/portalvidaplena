    ...
    <div className="p-8 space-y-6 bg-slate-50/50 min-h-screen animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">Gestão de Vendas (Equipe)</h1>
          <p className="text-slate-500 font-medium mt-2">
            Pool colaborativo de atendimentos. Puxe leads, converse e converta juntos.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          <div className="flex bg-white p-1.5 rounded-[1.25rem] shadow-sm border border-slate-200">
            <button 
              ...
              className={cn("p-2 rounded-xl transition-all duration-300", viewMode === "table" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-indigo-600")}
            >
              <List size={20} />
            </button>
            <button 
              onClick={() => setViewMode("kanban")}
              className={cn("p-2 rounded-xl transition-all duration-300", viewMode === "kanban" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-indigo-600")}
            >
              <LayoutGrid size={20} />
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "Total Leads", value: leads.length, icon: UserCheck, color: "blue" },
          { label: "Em Atendimento", value: leads.filter(l => l.status === "CONTACTED").length, icon: MessageSquare, color: "indigo" },
          { label: "Vendas Fechadas", value: leads.filter(l => l.status === "CLOSED").length, icon: CreditCard, color: "emerald" },
          { label: "Meus Leads", value: leads.filter(l => l.assignedToId === user.id).length, icon: UserPlus, color: "purple" },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5 hover:border-indigo-100 transition-all group">
      ...
      <div className="p-1 px-4 bg-white rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center min-h-[80px]">
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
           <table className="w-full text-left">
              <thead className="bg-slate-50/80 border-b border-slate-100">
                <tr>
                  <th className="px-8 py-5 font-black text-xs uppercase tracking-widest text-slate-400">Cliente</th>
                  <th className="px-8 py-5 font-black text-xs uppercase tracking-widest text-slate-400">Atendimento</th>
                    ...
                    <td className="px-8 py-5">
                      {lead.assignedTo ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className={cn("text-xs font-bold", lead.assignedTo.id === user.id ? "text-emerald-700" : "text-slate-500")}>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-black text-amber-600 tracking-widest uppercase">Fila Geral</span>
                        </div>
                      )}
                    </td>
                    <td className="px-8 py-5">
                      <span className={cn("px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-sm", statusStyles[lead.status as keyof typeof statusStyles])}>
                    ...
                    <td className="px-8 py-5 text-right flex justify-end gap-2">
                       {(!lead.assignedTo || lead.assignedTo.id !== user.id) && (
                         <button 
                          ...
                          className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-xs hover:bg-indigo-600 hover:text-white transition-all shadow-sm flex items-center gap-1"
                         >
                       ...
                       <button 
                        ...
                        className="px-4 py-2 bg-slate-50 text-slate-600 rounded-xl font-bold text-xs hover:bg-slate-200 transition-all shadow-sm"
                       >
                         Detalhes & Chat
                       </button>
                    </td>
      ...
      {/* Modal Lateral (Slide-over) de Detalhes do Lead (Chat / Transferência) */}
