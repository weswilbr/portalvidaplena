...
const quotes = [
  ...
  "Sua rede cresce na exata medida das pessoas que você conecta.",
...
export default function DashboardClient() {
  ...
  const stats = [
    { label: "Convidados Novos (Mês)", value: data?.stats?.newLeadsMonthly || 0, icon: UserPlus, color: "bg-blue-600" },
    { label: "Life Points (LP)", value: data?.stats?.totalLP?.toLocaleString("pt-BR") || 0, icon: TrendingUp, color: "bg-sky-500" },
    { label: `Meta ${data?.mainGoal?.title || ""}`, value: `${Math.round(data?.stats?.goalProgress || 0)}%`, icon: Target, color: "bg-blue-800" },
    { label: "Taxa de Conversão", value: `${data?.conversionRate || 0}%`, icon: Zap, color: "bg-cyan-500" },
    { label: "Bônus Construtor", value: data?.stats?.bonusPhase || "Fase 1", icon: Award, color: "bg-blue-500" },
  ];

  return (
    <div className="p-3 md:p-4 space-y-3 md:space-y-5 max-w-full overflow-hidden animate-in fade-in duration-700">
      {/* Hero Banner */}
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-900 via-blue-800 to-blue-600 rounded-2xl md:rounded-3xl p-4 md:p-8 text-white shadow-xl shadow-blue-200/50">
        <div className="absolute -top-10 -right-10 w-32 md:w-48 h-32 md:h-48 bg-white/10 rounded-full blur-2xl"></div>
        <div className="absolute -bottom-10 -left-10 w-40 md:w-64 h-40 md:h-64 bg-cyan-400/20 rounded-full blur-2xl"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
          <div className="space-y-2 md:space-y-3 text-center md:text-left w-full">
            <div className="inline-flex items-center gap-1.5 bg-white/20 w-fit px-3 py-1 rounded-full text-[9px] md:text-[10px] font-black backdrop-blur-sm border border-white/10 uppercase tracking-wider">
              ...
              Vida Plena em Movimento
            </div>
            <h1 className="text-base md:text-2xl lg:text-4xl font-black tracking-tight leading-tight">
              Construindo uma Rede que <span className="text-cyan-300">Multiplica Resultados</span>
            </h1>
          </div>
            ...
            {data?.recentLeads?.slice(0, 3).map((lead: any) => (
              <div key={lead.id} className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-white/30 overflow-hidden bg-white/10 flex items-center justify-center shadow-md" title={lead.name}>
                {lead.profilePic ? (
                  ...
                  <span className="text-white font-black text-xs">{lead.name?.charAt(0) || 'C'}</span>
                )}
              </div>
            ))}
            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-white/50 bg-blue-500 flex items-center justify-center text-[9px] md:text-[10px] font-black shadow-md">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1 text-xs md:text-sm hidden md:block">Evolução em tempo real da sua rede Vida Plena.</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setIsModalOpen(true)}
            className="hidden md:flex items-center gap-2 bg-blue-700 text-white px-4 py-2 rounded-xl font-bold shadow-md hover:bg-blue-800 transition-all"
          >
            <Plus size={16} />
            Novo Convidado
          </button>
            ...
            <div className="w-full aspect-[16/7] flex items-end justify-between gap-6 px-4">
              {data?.evolutionData?.map((item: any, i: number) => (
                <div key={i} className="flex-1 group relative">
                  <div
                    className="w-full bg-gradient-to-t from-blue-700 to-sky-400 rounded-2xl transition-all duration-700 hover:from-blue-800 cursor-pointer shadow-lg shadow-blue-100"
              ...
              {data?.sellerPerformance?.map((seller: any, i: number) => (
                <div key={i} className="space-y-4">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-sm font-black text-blue-700 uppercase tracking-widest">{seller.rate}% Conversão</span>
                  </div>
                  <div className="h-4 bg-slate-100 rounded-full overflow-hidden p-1 border border-slate-50 shadow-inner">
                    <div
                      className="h-full bg-gradient-to-r from-blue-700 to-sky-400 rounded-full transition-all duration-1000"
                      style={{ width: `${seller.rate}%` }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
                    <span>{seller.leads} Convidados Atendidos</span>
                    <span className="text-emerald-500">{seller.converted} Cadastros Feitos</span>
                  </div>
                </div>
              ))}
              {(!data?.sellerPerformance || data.sellerPerformance.length === 0) && (
                <p className="text-center text-slate-400 font-bold py-10 uppercase tracking-widest text-xs">Aguardando os primeiros cadastros...</p>
              )}
            </div>
          </div>

          {/* Dica */}
          <div className="p-10 bg-blue-700 rounded-[3rem] text-white shadow-2xl shadow-blue-100 relative overflow-hidden">
            <div className="relative z-10 flex items-center justify-between">
              <div className="space-y-4">
                <h4 className="font-black text-blue-200 flex items-center gap-2 italic text-sm uppercase tracking-widest">
                  <Sparkles size={18} />
                  Dica do Dia
                </h4>
                <p className="text-xl font-bold max-w-md leading-relaxed">
                  "{randomQuote || "Foque em conectar pessoas: sua rede cresce com relacionamento, não com pressa."}"
                </p>
              </div>
        ...
        <div className="space-y-8">
          <div className="p-10 bg-slate-900 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              <h4 className="text-slate-500 font-black uppercase tracking-[0.2em] text-[10px]">Próxima Meta de Rank</h4>
              <p className="text-4xl font-black mt-4 text-cyan-400 tracking-tight leading-none">{data?.mainGoal?.title}</p>
              ...
              <div className="mt-12 space-y-8 text-white">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-6 bg-white/5 rounded-[2rem] border border-white/10 hover:bg-white/10 transition-all">
                    <Users size={24} className="text-sky-400 mb-3" />
                    <p className="text-3xl font-black">{data?.mainGoal?.currentSignups || 0}</p>
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Cadastrados</p>
                  </div>
                  <div className="p-6 bg-white/5 rounded-[2rem] border border-white/10 hover:bg-white/10 transition-all">
                    <Target size={24} className="text-cyan-400 mb-3" />
                    <p className="text-3xl font-black">{data?.stats?.totalLeads || 0}</p>
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Na Rede</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="p-10 bg-white rounded-[3rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-6">
              {data?.upcomingActions?.map((action: any, i: number) => (
                <div key={i} className="flex items-center gap-5 group cursor-pointer">
                  <div className={cn("w-2 h-12 rounded-full", i === 0 ? "bg-blue-700" : i === 1 ? "bg-sky-400" : "bg-cyan-400")}></div>
                  <div className="flex-1">
                    <p className="font-bold text-slate-900 group-hover:text-blue-700 transition-colors">{action.title}</p>
            ...
            <Link
              ...
              className="w-full mt-10 py-5 rounded-3xl bg-slate-50 text-slate-900 font-black text-[10px] uppercase tracking-[0.2em] hover:bg-blue-700 hover:text-white transition-all text-center block shadow-sm"
            >
              VISUALIZAR AGENDA
            </Link>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-300">
            <div className="flex items-center justify-between mb-8 text-slate-900">
              <h2 className="text-3xl font-black tracking-tight">Novo Convidado</h2>
            ...
            <form className="space-y-6" onSubmit={handleSaveLead}>
              ...
              <button
                type="submit"
                className="w-full bg-blue-700 text-white py-6 rounded-3xl font-black text-xl shadow-2xl shadow-blue-200 hover:bg-blue-800 transition-all mt-4"
              >
                CADASTRAR CONVIDADO
              </button>
            </form>
